...
my_model.classes = ['tim', 'ao', 'banhkeo', 'bep', 'botgiat', 'ca', 'cathu', 'cahoi', 'caphe', 'carot', 'cha',
                    'chao', 'congcu', 'dau', 'dep', 'dochoi', 'dodonghop', 'dogiadung', 'dohop',
                    'douong', 'dung', 'sai', 'chinhxac','khong','gao', 'gaolut', 'gaonep', 'giavi', 'giay', 'khoaitay', 'mi', 'migoi',
                    'mypham', 'noi', 'phukien', 'quan', 'rau', 'raucu', 'sach', 'sua', 'suabot', 'suatuoi',
                    'thit', 'thitbo', 'thitga', 'thitheo', 'traicay', 'traitao', 'trangsuc', 'trung', 'xabong',
                    'xinchao', 'xucxich','_unknown_']
...
def audio_augmentation_pipeline(
    ...
    for i, (audio_path, labels) in enumerate(zip(path_batch, label_batch)):
        ...
        if audio_augmentations is None:
            temp = speech_commands_v2.load_clean_data()
            dataset_dir = speech_commands_v2.load_clean_data()
            print('path 1:' + dataset_dir)
...
def audio_augmentation_pipeline(
    ...
    for i, (audio_path, labels) in enumerate(zip(path_batch, label_batch)):
        ...
        audio_augmentations = globals().get('audio_augmentations', None)
        if audio_augmentations is None:
            #temp = speech_commands_v2.load_clean_data(dest_dir=dataset_dir)
            dataset_dir = speech_commands_v2.load_clean_data()
            #print('path 1:' + temp)
            audio_augmentations = audiomentations.Compose(
                p=1.0,
